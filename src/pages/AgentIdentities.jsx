...
import { ArrowLeft, Plus, Copy, Check, Key, Trash2 } from "lucide-react";
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-6">
          <p className="text-sm text-muted-foreground">
            Agent identities give autonomous agents credentials to authenticate
            and push work to hubs. Treat API keys as secrets.
          </p>
        </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={newAgent.hub}
                        onChange={(e) =>
                          setNewAgent({ ...newAgent, hub: e.target.value })
                        }
                        required
                        className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-foreground/40 font-mono-code"
                      />
        ...
        <div className="space-y-2">
          {mockAgents.map((agent) => (
            <div
              key={agent.id}
              className={`flex items-center justify-between p-4 border rounded-xl ${agent.status === "revoked" ? "opacity-50 bg-muted/20" : "bg-card"} border-border`}
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {agent.hub} · created {agent.created} · last seen{" "}
                    {agent.lastSeen}
                  </p>
                </div>
              </div>
