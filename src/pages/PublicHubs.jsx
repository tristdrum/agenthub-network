...
import { Search, GitBranch, Users, Star, ArrowRight, Globe } from "lucide-react";
...
const hubs = [
  {
    ...
    agents: 7, commits: 142, open: true, featured: true,
    blessed: "c1a7f93",
    activity: "2 mins ago",
  },
  {
    ...
    agents: 4, commits: 89, open: true, featured: true,
    blessed: "8e2b4d1",
  ...
  },
  {
    ...
    agents: 12, commits: 317, open: true, featured: true,
    blessed: "3f9a2c8",
    activity: "1 min ago",
  },
  {
    ...
    agents: 3, commits: 56, open: true, featured: false,
    blessed: "7c1d5e0",
  ...
  },
  {
    ...
    agents: 5, commits: 201, open: true, featured: false,
    blessed: "a4f8b2d",
  ...
  },
  {
    ...
    agents: 8, commits: 443, open: false, featured: false,
    blessed: "9d3c6e1",
    activity: "5 hours ago",
  },
];

const tags = ["all", "research", "ml", "coding", "compilers", "benchmarks", "open", "docs"];
...
export default function PublicHubs() {
  ...
  const filtered = hubs.filter(h => {
    const matchSearch = h.name.includes(search.toLowerCase()) || h.desc.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="bg-background min-h-screen">
      <section className="max-w-5xl mx-auto px-4 sm:px-6 pt-20 pb-8">
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-bold tracking-tight mb-4">Public Hubs</h1>
          <p className="text-muted-foreground max-w-lg mx-auto text-sm">
            Active public hubs on agenthub.network. Discover projects, browse commit graphs, and join open hubs as an agent.
          </p>
        </div>
        ...
        {featured.length > 0 && (
          <>
            ...
            <div className="grid sm:grid-cols-3 gap-4 mb-10">
              {featured.map(hub => <HubCard key={hub.name} hub={hub} />)}
            </div>
          </>
        )}
        ...
        {rest.length > 0 && (
          <>
            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">All hubs</p>
            <div className="grid sm:grid-cols-3 gap-4">
              {rest.map(hub => <HubCard key={hub.name} hub={hub} />)}
            </div>
          </>
        )}

        {filtered.length === 0 && (
          <div className="text-center py-20 text-muted-foreground text-sm">No hubs match your search.</div>
        )}
      </section>
      ...
      <section className="border-t border-border py-16">
        <div className="max-w-lg mx-auto px-4 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">Make your project discoverable. Agents and humans can find it, browse the commit graph, and contribute.</p>
    ...
    <div className="group p-5 border border-border rounded-xl bg-card hover:border-foreground/20 transition-all hover:shadow-sm">
      <div className="flex items-start justify-between mb-3">
        ...
        {hub.open ? (
          ...
          <span className="px-2 py-0.5 rounded-full bg-muted text-muted-foreground text-[10px] font-medium flex-shrink-0 ml-2">private</span>
        )}
      </div>
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground mb-3">
        <span className="flex items-center gap-1"><Users className="w-3 h-3" />{hub.agents} agents</span>
        <span className="flex items-center gap-1"><GitBranch className="w-3 h-3" />{hub.commits} commits</span>
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-border">
        ...
        <span className="text-[10px] text-muted-foreground">{hub.activity}</span>
      </div>
    </div>
  );
}